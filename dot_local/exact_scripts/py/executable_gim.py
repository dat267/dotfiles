...
Resolves shorthand to github.com/dat267/<repo>@<version> (subpaths like
adl/tools work; full module paths pass through unchanged). GOPRIVATE
scopes proxy/sumdb bypass to the module owner; a git insteadOf rewrite
in the subprocess env turns the https fetch into SSH (agent key auth)
so nothing prompts. All plumbing lives in the subprocess env — no
global git config is touched.
...
USAGE = "usage: gim.py <repo[@version]>   e.g. gim.py adl/tools@v0.3"
...
def main(argv=None):
    ...
    if not argv:
        print(USAGE)
        return 2
    module, version, owner = resolve_spec(argv[0])
