...
local function get_selected()
	return ya.sync(function()
		...
		for _, f in ipairs(cx.active.current.files) do
			if f.selected then
				files[#files + 1] = { url = tostring(f.url), ext = f.url:ext() or "", name = tostring(f.url:name()), parent = tostring(f.url:parent()) }
			end
		end
		return files
	end)()
...
function M:entry()
	...
	if #files < 2 then
		return ya.notify({ title = "Concat", content = "Select >= 2 files", timeout = 3 })
	end

	local is_flac = true
	for _, f in ipairs(files) do
		if f.ext:lower() ~= "flac" then is_flac = false; break end
	end

	local ext = files[1].ext ~= "" and files[1].ext or "mkv"
