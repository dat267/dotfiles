...
function M:entry()
	...
	if #files < 2 then
		return ya.notify({ title = "Concat", content = "Select at least 2 files", timeout = 3 })
	end

	local is_flac = true
	for _, f in ipairs(files) do
		local ext = f.url:ext() or ""
		if ext:lower() ~= "flac" then is_flac = false; break end
	end

	local first = tostring(files[1].url)
	local ext = files[1].url:ext() or "mkv"
	local parent = tostring(files[1].url:parent())
	local parent_name = parent:match("[^/]+$") or "output"
	local default = parent_name .. "_combined." .. ext
	...
	if not out:find("%.") then out = out .. "." .. ext end

	local n = #files
	local cmd = { "ffmpeg" }
	for _, f in ipairs(files) do
		cmd[#cmd + 1] = "-i"
		cmd[#cmd + 1] = tostring(f.url)
	end

	if is_flac then
		cmd[#cmd + 1] = "-filter_complex"
		cmd[#cmd + 1] = "concat=n=" .. n .. ":v=0:a=1"
		cmd[#cmd + 1] = "-c:a"
		cmd[#cmd + 1] = "flac"
	else
		local filter = ""
	...
	ya.notify({ title = "Concat", content = "Concatenating " .. n .. " files...", timeout = 5 })
