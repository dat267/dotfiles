...
function M:job()
	...
	if #files == 0 then
		return ya.notify({ title = "Compress", content = "No files selected", timeout = 3 })
	end
	...
	if format ~= "z" and format ~= "7" then
		return ya.notify({ title = "Compress", content = "Invalid format (z or 7)", timeout = 3 })
	end
	...
	local child = Command("7z"):args(args):stdout(Command.PIPED):stderr(Command.PIPED):spawn()
	ya.notify({ title = "Compress", content = "Compressing " .. #files .. " files...", timeout = 5 })
