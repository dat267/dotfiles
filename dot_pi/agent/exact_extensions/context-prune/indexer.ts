...
import type { CapturedBatch, IndexEntryData, ToolCallRecord } from "./types.ts";
import { CUSTOM_TYPE_INDEX, CUSTOM_TYPE_SUMMARY } from "./types.ts";
import {
...
/**
 * Runtime index of summarized tool calls. Persisted across restarts via
 * pi.appendEntry(CUSTOM_TYPE_INDEX, ...); rebuilt on session_start / session_tree.
 */
export class ToolCallIndexer {
  ...
  /** Rebuilds the in-memory index from session history. */
  ...
  /** Returns true if the given toolCallId has been summarized. */
  ...
  /** Registers short aliases so recovery queries can resolve t1 → real id. */
  ...
  /** Allocates short aliases (t1, t2, …) for a batch's tool calls. */
  ...
  /** Look up a single record by toolCallId or short alias. */
