...
/**
 * Trims a captured batch down to tool calls not yet summarized, honoring the
 * frontier's last-attempted boundary so a skipped-oversized prune range is not
 * retried forever and later tool calls from the same long turn still survive.
 */
export function trimBatchToPendingRange(
  batch: CapturedBatch,
  currentFrontier: PruneFrontier | null,
  indexer: { isSummarized(id: string): boolean },
): CapturedBatch | null {
  let toolCalls = batch.toolCalls.filter((tc) => !indexer.isSummarized(tc.toolCallId));
